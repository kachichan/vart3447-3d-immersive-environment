...
18. Right-click it → **Delete**. Do the same to **Chair** (or keep it,
    if your room had a chair about that size).
19. Right-click **Root** in the Hierarchy → **New Entity** → **3D** →
    **Box**. A 1 m cube appears.
20. In the Inspector, set its **Scale** to: *your wall length*, **2.5**,
    **0.1**. (For example: 4, 2.5, 0.1.)
21. Set its **Position** to **0, 1.25, -2**. (y is half its height, so
    it stands on the floor instead of sinking into it. z = -2 puts it two
    metres in front of where you'll be standing — you start at 0, 0, 0.)
22. Drag a colour from the **Assets** area onto the box.
23. In the strip of icons down the left edge, click **Publish /
    Download** (an arrow going into a box, near the bottom). A **Builds**
    panel opens. Click **Publish** (top right), then the orange
    **Publish** button at the bottom of the form. Wait until it finishes.
24. Copy the link under **Primary build**. It looks like
    `playcanv.as/p/xxxxxxxx/`. This link is yours for the whole course.
    **Publishing again does not change what's at the link by itself:**
...
27. To make a new box: right-click **Root** → New Entity → 3D → Box. To
    copy one you've already made: select it, **Ctrl/Cmd + D**.
...
30. **At 5:00 pm, publish again** (step 23) whether you're finished or
    not, and **Set Primary Build** on the new build. Your link stays the
    same, so your block in the channel is already up to date.
...
33. What's wrong? Say it out loud to the person next to you. Don't take
    the headset off yet.
34. Headset off. Fix the two worst things. Publish again, Set Primary
    Build. Reload the page in the headset (tap the refresh arrow).
...
| Everything is dark, or one flat colour fills my view | You're standing inside a box | You start at 0, 0, 0. Move the box away from the origin (change its Position), or move it so the origin is inside the *room* but not inside a wall. Also check the **Light** entity is still in the Hierarchy. |
| Boxes half-sunk into the floor | Position y is 0 | Position is the box's centre. Set y to half the box's height. |
| I changed something and the headset shows the old version | You published but didn't **Set Primary Build**, or the old page is cached | Open the Builds panel: the top build in the history should say **PRIMARY**. If not, **⋯** → **Set Primary Build**. Then reload in the headset. |
